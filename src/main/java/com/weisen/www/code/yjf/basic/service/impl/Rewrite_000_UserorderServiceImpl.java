...
//                        String payWay = Rewrite_Constant.ACCOUNTTYPE_ALIPAY;
                        // 根据解析出来的数据验证订单，用户
                        List<Userorder> userorders = userorderRepository.findByOrdercodes(tradeNo);
...
//                        createFlow(userorder);
    ...
    public Result weChatRefundNotifys(Map<String, String> map) {
        System.out.println("微信开始回调");
        log.debug("微信支付開始回調");
        // 解析微信服务器回调过来的数据。
        String result_code = "";
...
//        total_fee = map.get("total_fee");
        if ("SUCCESS".equals(result_code) && "SUCCESS".equals(return_code)) {
            ...
            System.out.println("微信调用成功");
            log.debug("微信回调开始");
            List<Userorder> userorders = userorderRepository.findByOrdercodes(out_trade_no);
            for (int i = 0; i < userorders.size(); i++) {
                Userorder userorder = userorders.get(i);
                //不是待支付订单不能支付
                if (!Rewrite_Constant.ORDER_WAIT_PAY.equals(userorder.getOrderstatus())) {
