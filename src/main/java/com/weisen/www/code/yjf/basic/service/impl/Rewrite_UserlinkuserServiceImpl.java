...
public class Rewrite_UserlinkuserServiceImpl implements Rewrite_UserlinkuserService {
	...
	public Rewrite_UserlinkuserServiceImpl(Rewrite_UserlinkuserRepository rewrite_UserlinkuserRepository,
			Rewrite_LinkuserRepository rewrite_LinkuserRepository,Rewrite_LinkaccountRepository rewrite_LinkaccountRepository) {
    ...
    public Result findAllByRecommendAndInfo(Long userid, int startPage, int pageSize) {
        ...
        list.forEach(x-> {
            ...
            if(linkuser.getPhone() != null && !"".equals(linkuser.getPhone())){
                rewrite_UserLink.setPhoneOrToken("元积分用户："+linkuser.getPhone().substring(0,3)
                    +"****"+linkuser.getPhone().substring(7,linkuser.getPhone().length()));
            }else{
