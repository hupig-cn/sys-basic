...
//	List<Receiptpay> getReceiptpayByUseridAndTime(String userId, String startTime, String endTime, String dealtype);
 ...
 // 根据类型，查询时间段内的收益数据
    ...
    // 根据时间,用户关系查询用户的明细
	@Query(value = "select * from receiptpay where userid= ?1 and dealtype != 6 and sourcer = ?2 and createdate between ?3 and  ?4", nativeQuery = true)
	List<Receiptpay> getReceiptpayByUseridAndSourcerAndTime(String userid,String sourcer,String firstTime ,String lastTime);
    
	
	// 根据用户关系查询用户的明细
	@Query(value = "select * from receiptpay where userid= ?1 and dealtype != 6 and sourcer = ?2", nativeQuery = true)
