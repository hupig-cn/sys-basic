...
public interface Rewrite_LinkuserRepository extends JpaRepository<Linkuser, Long> {
    ...
    @Query(value = "select l.userid as userid ,l.phone as phone ,l.name as name , a.balance as balance,a.integral as integral , (case when l.paypassword IS NOT NULL THEN '已设置' ELSE '未设置' END ) as password," +
        ...
        "where (:userName IS NULL OR :userName = l.userid) AND (CASE WHEN :realName IS NULL THEN 1 = 1  WHEN :realName = 1 THEN l.name IS NOT NULL  ELSE  l.name IS NULL END ) ORDER BY l.createdate DESC LIMIT :pageNum,:pageSize ",nativeQuery = true)
    ...
    @Query(value = "select count(*) from linkuser l RIGHT JOIN userassets a ON l.userid = a.userid JOIN userlinkuser ulu ON ulu.userid = l.userid " +
        "where (:userName IS NULL OR :userName = l.userid) AND (CASE WHEN :realName IS NULL THEN 1 = 1  WHEN :realName = 1 THEN l.name IS NOT NULL  ELSE  l.name IS NULL END ) ORDER BY l.createdate DESC",nativeQuery = true)
