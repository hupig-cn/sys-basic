...
public interface Rewrite_LinkuserRepository extends JpaRepository<Linkuser, Long> {
    ...
    @Query(value = "select l.id as id ,l.phone as name , a.balance as balance,a.integral as integral , (case when l.paypassword IS NOT NULL THEN '已设置' ELSE '未设置' END ) as password," +
        "(CASE WHEN l.name IS NOT NULL THEN '已实名' ELSE '未实名' END ),l.createdate as createtime ,ulu.recommendid as recommendid from linkuser l RIGHT JOIN userassets a ON l.userid = a.userid JOIN userlinkuser ulu ON ulu.userid = l.userid " +
